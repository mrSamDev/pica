...
const app = Fastify({
  logger: {
    level: "info",
    redact: {
      paths: ["req.headers.authorization", "req.headers.cookie", "body.password", "body.creditCard", "*.password", "*.secret", "*.token"],
      censor: "[REDACTED]",
    },
  },
});
...
const streams = [{ stream: process.stdout }, { stream: createWriteStream("./app.log") }, { level: "error", stream: createWriteStream("./error.log") }];
