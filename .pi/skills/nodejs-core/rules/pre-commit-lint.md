...
invocation only covers _staged_ changes, which is not what CI checks):
...
**Always pass `--no-validate-metadata`.** Metadata validation is _on by
default_ (`-V, --validate-metadata`), and it enforces the `PR-URL:` and
...
Validate _with_ metadata only when inspecting an already-landed commit:
