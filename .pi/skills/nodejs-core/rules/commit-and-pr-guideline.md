...
- **Aim for 50 columns, never exceed 72.** `core-validate-commit` warns above
  50 and _fails_ above 72. A `Revert "..."` subject gets 9 extra columns.
...
trigger is whether a reader can tell _why_ from the subject alone.
...
Validate _with_ metadata only when inspecting a commit that has already landed
on `main`:
