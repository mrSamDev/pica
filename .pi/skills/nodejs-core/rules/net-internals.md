...
const sockets = handles.filter((h) => h.constructor.name === "TCP" || h.constructor.name === "Socket");
...
setInterval(() => {
  ...
  for (const socket of connections) {
    console.log(`  ${socket.remoteAddress}:${socket.remotePort} - ` + `rx: ${socket._bytesReceived}, tx: ${socket._bytesSent}`);
  }
}, 5000);
```
