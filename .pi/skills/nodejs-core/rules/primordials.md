...
const { ArrayIsArray, ArrayPrototypeJoin, ArrayPrototypePush, ObjectDefineProperty, ObjectKeys, SafeSet, StringPrototypeSlice, StringPrototypeStartsWith, TypeError } = primordials;
```
...
```javascript
const { SafeMap, SafeSet, SafeWeakMap, SafeWeakSet, SafeWeakRef, SafeFinalizationRegistry, SafeArrayIterator, SafeStringIterator } = primordials;
```
